...
class MainActivity: FlutterFragmentActivity() {
    ...
    private fun handleIntentAction(intent: Intent?) {
        val action = intent?.getStringExtra("action") ?: intent?.getStringExtra("open_screen")
        if (action != null) {
            pendingAction = action
            try {
                flutterEngine?.dartExecutor?.binaryMessenger?.let { messenger ->
                    MethodChannel(messenger, "com.linkary/quick_notification")
                        .invokeMethod("onActionReceived", action)
                }
            } catch (_: Exception) {}
        }
    }
            ...
            when (call.method) {
                "showCustomNotification" -> {
                    try {
                        ...
                        // 💾 حفظ session_id في SharedPreferences حتى يتمكن WidgetActionReceiver من استخدامها
                        if (!sessionId.isNullOrBlank()) {
                            getSharedPreferences(NotificationHelper.PREFS_NAME, Context.MODE_PRIVATE)
                                .edit()
                                .putString(WidgetActionReceiver.SESSION_ID_KEY, sessionId)
                                .apply()
                        }

                        // 💾 حفظ آخر حالة للإشعار لإعادة إنشائه بعد إعادة تشغيل الجهاز
                        NotificationHelper.saveNotifState(
                        ...
                        // 🔔 عرض الإشعار عبر NotificationHelper (مع DecoratedCustomViewStyle وأزرار وظيفية)
    ...
    // تم نقل createQuickNotificationChannel إلى NotificationHelper.createChannel()
